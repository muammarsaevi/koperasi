...
class Lap_tagihan extends OperatorController {
	...
	public function index() {
		...
		$this->data['isi'] = $this->load->view('lap_tagihan_list_v', $this->data, TRUE);
	...
	function ajax_list() {
		...
		foreach ($data['data'] as $r) {
			...
			$rows[$i]['lama_angsuran_txt'] = $r->lama_angsuran.' Angsuran';
			...
			$rows[$i]['angsur_ke'] = number_format($r->bln_sudah_angsur).' Kali ';
			$rows[$i]['angsuran_bln'] = number_format(nsi_round($r->ags_per_bulan));
			
			// Jatuh Tempo
			$angsur=explode(' ', $r->tgl_angsuran);
			$txt_angsur = jin_date_ina($angsur[0],'p');

			$sdh_ags_ke = $r->bln_sudah_angsur;
			$ags_ke = $r->bln_sudah_angsur + 1;

			$denda_hari = $data_bunga_arr['denda_hari'];
			$tgl_pinjam = substr($r->tgl_pinjam, 0, 7);
			$tgl_pinja = substr($r->tgl_angsuran, 0, 10);
			$tgl_tempo = date('Y-m-d', strtotime("+".$ags_ke." month", strtotime($tgl_pinjam)));
			$tgl_tempo = substr($tgl_tempo, 0, 7) . '-' . sprintf("%02d", $denda_hari);
			$txt_status = '';
			$txt_status_tip = 'Ags Ke: ' . $ags_ke . ' Tempo: ' . $tgl_pinja;
			if($tgl_pinja < date('Y-m-d')) {
				$rows[$i]['merah'] = 1;
			...
			$rows[$i]['status'] = $txt_angsur;
			$rows[$i]['ceksaja'] = $txt_status_tip;
