...
class Angsuran extends OperatorController {
	...
	function ajax_list($id = NULL) {
		...
		foreach ($data['data'] as $r) {
			...
			$tgl_tempoo = date('Y-m-d', strtotime("+".$r->angsuran_ke."weeks", strtotime($tgl_pinjam)));
			$tgl_tempo = date('Y-m-d', strtotime("+".$r->angsuran_ke."weeks", strtotime($tgl_tempoo)));
			$tgl_bayar  = substr($r->tgl_bayar, 0, 10);
			$data_bunga_arr = $this->bunga_m->get_key_val();
			$denda_hari = $data_bunga_arr['denda_hari'];

			$tgl_tempo_max = date('Y-m-d', strtotime("+".($denda_hari )." days", strtotime($tgl_tempo)));

			$tgl_tempo_h = str_replace('-', '', $tgl_tempo);
